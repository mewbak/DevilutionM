...
void __fastcall DrawSpellCel(int xp, int yp, char *Trans, int nCel, int w)
{
    ...
    output = (char *)gpBuffer + screen_y_times_768[yp] + xp;
    if (nCel > 41)
        return;
    ...
    for (y = 0; y < w; y++) {
        for (x = 0; x < w; x++) {
            *output = Trans[nCel * (w + 1) * w + y * (w + 1) + x + offset];
            output++;
        }
        output = &output[-w - 768];
    }

//    int *pFrameTable = (int *)&Trans[4 * nCel];
//    BYTE *pCel = &Trans[*pFrameTable];
//    BYTE *pDecodeTo = &gpBuffer[xp + LineTbl[yp]];
//    BYTE *pBuffEnd = &pCel[pFrameTable[1] - *pFrameTable];
//    BYTE *tbl = STransTbl;
//
//    for( ; pCel != pBuffEnd; pDecodeTo -= w+768) {
//        for(int i = w; i > 0;) {
//            BYTE width = *pCel++;
//            if ( (width & 0x80u) == 0 ) {
//                i -= width;
//                asm_cel_light_edge(width, tbl, pDecodeTo, pCel);
//            }
//            else {
//                width = -(char)width;
//                pDecodeTo += width;
