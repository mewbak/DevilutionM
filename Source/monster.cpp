...
void GetLevelMTypes()
{
	...
	if (!setlevel) {
		...
		if (gbMaxPlayers != 1 && currlevel == quests[QTYPE_KING]._qlevel) {
			...
			AddMonsterType(skeltypes[xrandom(88, nt)], 1);
		}
...
void InitMonster(int i, int rd, int mtype, int x, int y)
{
	...
	monster[i]._mAnimCnt = xrandom(88, monst->Anims[MA_STAND].Rate - 1);
	monster[i]._mAnimLen = monst->Anims[MA_STAND].Frames;
	monster[i]._mAnimFrame = xrandom(88, monst->Anims[MA_STAND].Frames - 1) + 1;

	if (monst->mtype == MT_DIABLO) {
		monster[i]._mmaxhp = (xrandom(88, 1) + 1666) << 6;
	} else {
		monster[i]._mmaxhp = (monst->mMinHP + xrandom(88, monst->mMaxHP - monst->mMinHP + 1)) << 6;
	}
...
void ClrAllMonsters()
{
	...
	for (i = 0; i < MAXMONSTERS; i++) {
		...
		Monst->_menemy = xrandom(89, gbActivePlayers);
...
void PlaceUniqueMonst(int uniqindex, int miniontype, int packsize)
{
	...
	if (Monst->_mAi != AI_GARG) {
		...
		Monst->_mAnimFrame = xrandom(88, Monst->_mAnimLen - 1) + 1;
...
void PlaceGroup(int mtype, int num, int leaderf, int leader)
{
	...
	for (try1 = 0; try1 < 10; try1++) {
		...
		j = 0;
		for (try2 = 0; j < num && try2 < 100; xp += offset_x[xrandom(94, 8)], yp += offset_x[xrandom(94, 8)]) {
			if (!MonstPlace(xp, yp)
			...
			if (leaderf & 1) {
				...
				if (monster[nummonsters]._mAi != AI_GARG) {
					...
					monster[nummonsters]._mAnimFrame = xrandom(88, monster[nummonsters]._mAnimLen - 1) + 1;
...
void InitMonsters()
{
	...
	if (!setlevel) {
		...
		while (nummonsters < totalmonsters) {
			mtype = scattertypes[xrandom(95, numscattypes)];
			if (currlevel == 1 || xrandom(95, 2) == 0)
				na = 1;
			else if (currlevel == 2)
				na = xrandom(95, 2) + 2;
			else
...
void M_StartHeal(int i)
{
	...
	Monst->_mVar1 = Monst->_mmaxhp / (16 * (xrandom(97, 5) + 4));
}
...
void M_TryM2MHit(int i, int mid, int hper, int mind, int maxd)
{
	...
	if (monster[mid]._mhitpoints >> 6 > 0 && (monster[mid].MType->mtype != MT_ILLWEAV || monster[mid]._mgoal != MGOAL_RETREAT)) {
		int hit = xrandom(4, 100);
		if (monster[mid]._mmode == MM_STONE)
			hit = 0;
		if (!CheckMonsterHit(mid, &ret) && hit < hper) {
			int dam = (mind + xrandom(5, maxd - mind + 1)) << 6;
...
void M_TryH2HHit(int i, int pnum, int Hit, int MinDam, int MaxDam)
{
	...
	dam = (MinDam << 6) + xrandom(99, (MaxDam - MinDam + 1) << 6);
...
void M_Teleport(int i)
{
	...
	if (Monst->_mmode != MM_STONE) {
		_mx = Monst->_menemyx;
		_my = Monst->_menemyy;
		rx = 2 * xrandom(100, 2) - 1;
		ry = 2 * xrandom(100, 2) - 1;
...
BOOL M_CallWalk2(int i, int md)
{
	...
	if (xrandom(101, 2)) { // Randomly go left or right
	...
	if (abs(mx) >= 2 || abs(my) >= 2) {
		if (v < 2 * Monst->_mint + 10) {
			if (abs(mx) >= 2 * Monst->_mint + 4 || abs(my) >= 2 * Monst->_mint + 4) {
				if (xrandom(104, 100) < 2 * Monst->_mint + 20) {
					md = xrandom(104, 8);
				}
				M_DumbWalk(i, md);
			} else {
...
void MAI_SkelSd(int i)
{
	...
	Monst->_mdir = md;
	if (abs(x) >= 2 || abs(y) >= 2) {
		if (Monst->_mVar1 == MM_DELAY || (xrandom(106, 100) >= 35 - 4 * Monst->_mint)) {
			M_CallWalk(i, md);
		} else {
			M_StartDelay(i, 15 - 2 * Monst->_mint + xrandom(106, 10));
		}
	} else {
		if (Monst->_mVar1 == MM_DELAY || (xrandom(105, 100) < 2 * Monst->_mint + 20)) {
			M_StartAttack(i);
		} else {
			M_StartDelay(i, 2 * (5 - Monst->_mint) + xrandom(105, 10));
		}
	}
...
void MAI_Snake(int i)
{
	...
	if (abs(mx) >= 2 || abs(my) >= 2) {
		if (abs(mx) < 3 && abs(my) < 3 && LineClearF1(PosOkMonst, i, Monst->_mx, Monst->_my, fx, fy) && Monst->_mVar1 != MM_CHARGE) {
		...
		} else if (Monst->_mVar1 == MM_DELAY || xrandom(106, 100) >= 35 - 2 * Monst->_mint) {
			...
			M_StartDelay(i, 15 - Monst->_mint + xrandom(106, 10));
		}
	} else {
		if (Monst->_mVar1 == MM_DELAY
		    || Monst->_mVar1 == MM_CHARGE
		    || (xrandom(105, 100) < Monst->_mint + 20)) {
			M_StartAttack(i);
		} else
			M_StartDelay(i, 10 - Monst->_mint + xrandom(105, 10));
	}
	if (Monst->_mmode == MM_STAND)
		Monst->_mAnimData = Monst->MType->Anims[MA_STAND].Data[Monst->_mdir];
}
...
void MAI_Bat(int i)
{
	...
	if (Monst->MType->mtype == MT_GLOOM
		...
		if (Monst->MType->mtype == MT_FAMILIAR) {
			AddMissile(Monst->_menemyx, Monst->_menemyy, Monst->_menemyx + 1, 0, -1, MIS_LIGHTNING, 1, i, xrandom(109, 10) + 1, 0);
		}
	}
...
void MAI_SkelBow(int i)
{
	...
	my = Monst->_menemyy;
	if (!walking) {
		if (xrandom(110, 100) < 2 * Monst->_mint + 3) {
...
void MAI_Fireman(int i)
{
	...
	if (Monst->_mgoal == MGOAL_NORMAL) {
		...
		if (Monst->_mgoalvar1 == 3) {
			...
			M_StartDelay(i, xrandom(112, 10) + 5);
			Monst->_mgoalvar1++;
		}
...
void MAI_Round(int i, BOOL special)
{
	...
	if (Monst->_mmode == MM_STAND && Monst->_msquelch != 0) {
		...
		if ((abs(mx) >= 2 || abs(my) >= 2) && Monst->_msquelch == UCHAR_MAX && dTransVal[Monst->_mx][Monst->_my] == dTransVal[fx][fy]) {
			if (Monst->_mgoal == MGOAL_MOVE || (abs(mx) >= 4 || abs(my) >= 4) && xrandom(115, 4) == 0) {
				if (Monst->_mgoal != MGOAL_MOVE) {
					Monst->_mgoalvar1 = 0;
					Monst->_mgoalvar2 = xrandom(116, 2);
				}
				...
				if (Monst->_mgoalvar1++ >= 2 * dist && DirOK(i, md) || dTransVal[Monst->_mx][Monst->_my] != dTransVal[fx][fy]) {
					...
					M_StartDelay(i, xrandom(125, 10) + 10);
				}
			}
		} else
			Monst->_mgoal = MGOAL_NORMAL;
		if (Monst->_mgoal == MGOAL_NORMAL) {
			if (abs(mx) >= 2 || abs(my) >= 2) {
				...
				Monst->_mdir = md;
				if (special && Monst->_mhitpoints < (Monst->_mmaxhp >> 1) && xrandom(117, 2) != 0)
					M_StartSpAttack(i);
				else
					M_StartAttack(i);
			}
...
void MAI_Ranged(int i, int missile_type, BOOL special)
{
	...
	if (Monst->_msquelch == UCHAR_MAX || Monst->_mFlags & MFLAG_TARGETS_MONSTER) {
		...
		if (Monst->_mVar1 == MM_RATTACK) {
			M_StartDelay(i, xrandom(118, 20));
		} else if (abs(mx) < 4 && abs(my) < 4) {
			if (xrandom(119, 100) < 10 * (Monst->_mint + 7))
				M_CallWalk(i, opposite[md]);
		}
		if (Monst->_mmode == MM_STAND) {
			if (LineClear(Monst->_mx, Monst->_my, fx, fy)) {
...
void MAI_RoundRanged(int i, int missile_type, BOOL checkdoors, int dam, int lessmissiles)
{
	...
	if (Monst->_mmode == MM_STAND && Monst->_msquelch != 0) {
		...
		v = xrandom(121, 10000);
		if ((abs(mx) >= 2 || abs(my) >= 2) && Monst->_msquelch == UCHAR_MAX && dTransVal[Monst->_mx][Monst->_my] == dTransVal[fx][fy]) {
			if (Monst->_mgoal == MGOAL_MOVE || ((abs(mx) >= 3 || abs(my) >= 3) && xrandom(122, 4 << lessmissiles) == 0)) {
				if (Monst->_mgoal != MGOAL_MOVE) {
					Monst->_mgoalvar1 = 0;
					Monst->_mgoalvar2 = xrandom(123, 2);
				}
				Monst->_mgoal = MGOAL_MOVE;
				if (abs(mx) > abs(my)) {
					dist = abs(mx);
		...
		if (Monst->_mmode == MM_STAND) {
			M_StartDelay(i, xrandom(125, 10) + 5);
		}
	}
}
	...
	if (Monst->_mmode == MM_STAND && Monst->_msquelch != 0) {
		...
		if (Monst->_mgoal == MGOAL_NORMAL) {
			if (((abs(mx) >= 3 || abs(my) >= 3) && v < 5 * (Monst->_mint + 2) || v < 5 * (Monst->_mint + 1) || Monst->_mgoalvar3 == 4) && LineClear(Monst->_mx, Monst->_my, fx, fy)) {
				...
				if (xrandom(124, 100) < 10 * (Monst->_mint + 4)) {
					Monst->_mdir = md;
					if (xrandom(124, 2) != 0)
						M_StartAttack(i);
					else
...
void MAI_SkelKing(int i)
{
	...
	if (Monst->_mmode == MM_STAND && Monst->_msquelch != 0) {
		...
		v = xrandom(126, 100);
		if ((abs(mx) >= 2 || abs(my) >= 2) && Monst->_msquelch == UCHAR_MAX && dTransVal[Monst->_mx][Monst->_my] == dTransVal[fx][fy]) {
			if (Monst->_mgoal == MGOAL_MOVE || (abs(mx) >= 3 || abs(my) >= 3) && xrandom(127, 4) == 0) {
				if (Monst->_mgoal != MGOAL_MOVE) {
					Monst->_mgoalvar1 = 0;
					Monst->_mgoalvar2 = xrandom(128, 2);
				}
				Monst->_mgoal = MGOAL_MOVE;
				if (abs(mx) > abs(my)) {
					dist = abs(mx);
			...
			if (gbMaxPlayers == 1
				...
				if (abs(mx) >= 2 || abs(my) >= 2) {
					...
					if (v >= Monst->_mint + 25
						...
						M_StartDelay(i, xrandom(130, 10) + 10);
					} else {
						M_CallWalk(i, md);
					}
...
void MAI_Rhino(int i)
{
	...
	if (Monst->_mmode == MM_STAND && Monst->_msquelch != 0) {
		...
		v = xrandom(131, 100);
		if (abs(mx) >= 2 || abs(my) >= 2) {
			if (Monst->_mgoal == MGOAL_MOVE || (abs(mx) >= 5 || abs(my) >= 5) && xrandom(132, 4) != 0) {
				if (Monst->_mgoal != MGOAL_MOVE) {
					Monst->_mgoalvar1 = 0;
					Monst->_mgoalvar2 = xrandom(133, 2);
				}
				Monst->_mgoal = 4;
				if (abs(mx) > abs(my)) {
					dist = abs(mx);
				...
				if (abs(mx) >= 2 || abs(my) >= 2) {
					...
					if (v >= 2 * Monst->_mint + 33
					    && (Monst->_mVar1 != MM_WALK && Monst->_mVar1 != MM_WALK2 && Monst->_mVar1 != MM_WALK3
					        || Monst->_mVar2
					        || v >= 2 * Monst->_mint + 83)) {
						M_StartDelay(i, xrandom(135, 10) + 10);
					} else {
						M_CallWalk(i, md);
					}
		...
		if (Monst->_mgoal == MGOAL_RETREAT) {
			...
			if (abs(mx) >= 2 || abs(my) >= 2) {
				if (v < 5 * (Monst->_mint + 10) && LineClear(Monst->_mx, Monst->_my, fx, fy)) {
					M_StartRAttack(i, counsmiss[Monst->_mint], Monst->mMinDamage + xrandom(77, Monst->mMaxDamage - Monst->mMinDamage + 1));
				} else if (xrandom(124, 100) < 30) {
					Monst->_mgoal = MGOAL_MOVE;
					Monst->_mgoalvar1 = 0;
					M_StartFadeout(i, md, FALSE);
				} else
					M_StartDelay(i, xrandom(105, 10) + 2 * (5 - Monst->_mint));
			} else {
				Monst->_mdir = md;
				if (Monst->_mhitpoints < (Monst->_mmaxhp >> 1)) {
				    ...
				    || xrandom(105, 100) < 2 * Monst->_mint + 20) {
