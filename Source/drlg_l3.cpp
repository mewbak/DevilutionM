...
void DRLG_L3CreateBlock(int x, int y, int obs, int dir)
{
	...
	blksizex = xrandom(0, 2) + 3;
	blksizey = xrandom(0, 2) + 3;

	if (dir == 0) {
		y2 = y - 1;
		y1 = y2 - blksizey;
		if (blksizex < obs) {
			x1 = xrandom(0, blksizex) + x;
		}
		if (blksizex == obs) {
			x1 = x;
		}
		if (blksizex > obs) {
			x1 = x - xrandom(0, blksizex);
		}
		x2 = blksizex + x1;
	}
	if (dir == 3) {
		...
		x1 = x2 - blksizex;
		if (blksizey < obs) {
			y1 = xrandom(0, blksizey) + y;
		}
		if (blksizey == obs) {
			y1 = y;
		}
		if (blksizey > obs) {
			y1 = y - xrandom(0, blksizey);
		}
		y2 = y1 + blksizey;
	}
	if (dir == 2) {
	...
	for (j = 0; j < 39; j++) {
		...
		for (i = 0; i < 37; i++) {
			if (dungeon[i][j] == 0 && dungeon[i][j + 1] == 1) {
				...
				if (xs > 3 && xrandom(0, 2) != 0) {
					for (k = xc; k < i; k++) {
						dungeon[k][j] = xrandom(0, 2);
					}
				}
				xs = 0;
			}
	...
	for (i = 0; i < 39; i++) {
		...
		for (j = 0; j < 37; j++) {
			if (dungeon[i][j] == 0 && dungeon[i + 1][j] == 1) {
				...
				if (ys > 3 && xrandom(0, 2) != 0) {
					for (k = yc; k < j; k++) {
						dungeon[i][k] = xrandom(0, 2);
					}
				}
				ys = 0;
			}
		...
		while (!bail && trys < 200) {
			...
			while (dircheck < 4 && riveramt < 100) {
				...
				if (dungeon[rx][ry] == 7) {
					dircheck = 0;
					if (dir < 2) {
						river[2][riveramt] = (BYTE)xrandom(0, 2) + 17;
					}
					if (dir > 1) {
						river[2][riveramt] = (BYTE)xrandom(0, 2) + 15;
					}
					river[0][riveramt] = rx;
					river[1][riveramt] = ry;
					riveramt++;
...
void DRLG_L3PlaceRndSet(const BYTE *miniset, int rndper)
{
	...
	for (sy = 0; sy < 40 - sh; sy++) {
		for (sx = 0; sx < 40 - sw; sx++) {
			...
			if (found == TRUE && xrandom(0, 100) < rndper) {
				for (yy = 0; yy < sh; yy++) {
					for (xx = 0; xx < sw; xx++) {
						if (miniset[kk] != 0) {
...
void DRLG_L3Wood()
{
	...
	for (j = 0; j < DMAXY - 1; j++) {
		for (i = 0; i < DMAXX - 1; i++) {
			if (dungeon[i][j] == 10 && xrandom(0, 2) != 0) {
				x = i;
			...
			if (dungeon[i][j] == 9 && xrandom(0, 2) != 0) {
				y = j;
				while (dungeon[i][y] == 9) {
					y++;
				}
			...
			if (dungeon[i][j] == 11 && dungeon[i + 1][j] == 10 && dungeon[i][j + 1] == 9 && xrandom(0, 2) != 0) {
				dungeon[i][j] = 125;
				x = i + 1;
				while (dungeon[x][j] == 10) {
					x++;
	...
	for (j = 0; j < DMAXY; j++) {
		for (i = 0; i < DMAXX; i++) {
			if (dungeon[i][j] != 7 || xrandom(0, 1) != 0 || !SkipThemeRoom(i, j)) {
				continue;
			}
			rt = xrandom(0, 2);
			if (rt == 0) {
