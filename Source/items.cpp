...
void GetStaffSpell(int i, int lvl, BOOL onlygood)
{
	...
	if (!xrandom(17, 4)) {
		...
		item[i]._iSpell = bs;
		item[i]._iCharges = minc + xrandom(19, maxc);
...
void GetItemAttrs(int i, int idata, int lvl)
{
	...
	item[i]._iAC = AllItemsList[idata].iMinAC + xrandom(20, AllItemsList[idata].iMaxAC - AllItemsList[idata].iMinAC + 1);
	...
	if (item[i]._itype == ITYPE_GOLD) {
		if (gnDifficulty == DIFF_NORMAL)
			rndv = 5 * currlevel + xrandom(21, 10 * currlevel);
		else
			rndv = lvl;

		if (gnDifficulty == DIFF_NIGHTMARE)
			rndv = 5 * (currlevel + 16) + xrandom(21, 10 * (currlevel + 16));
		if (gnDifficulty == DIFF_HELL)
			rndv = 5 * (currlevel + 32) + xrandom(21, 10 * (currlevel + 32));
...
{
	return param1 + xrandom(22, param2 - param1 + 1);
}
...
int RndItem(int m)
{
	...
	if (xrandom(24, 100) > 40)
		return 0;

	if (xrandom(24, 100) > 25)
		return 1;
...
void ItemRndDur(int ii)
{
	if (item[ii]._iDurability && item[ii]._iDurability != DUR_INDESTRUCTIBLE)
		item[ii]._iDurability = xrandom(0, item[ii]._iMaxDur >> 1) + (item[ii]._iMaxDur >> 2) + 1;
}
...
void SetupAllItems(int ii, int idx, int iseed, int lvl, int uper, int onlygood, int recreate, int pregen)
{
	...
	if (item[ii]._iMiscId != IMISC_UNIQUE) {
		iblvl = -1;
		if (xrandom(32, 100) > 10 && xrandom(33, 100) > lvl || (iblvl = lvl, lvl == -1)) {
