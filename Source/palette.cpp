...
void LoadPalette(char *pszFileName)
{
	...
	PaletteFadeIn(8);//Change this is a hack for force the palette to apply.
...
{	
	...
	if (l == DTYPE_TOWN) {
		...
		printf("LOADING TOWN PAL\n\n");
	} else {
		sprintf(szFileName, "Levels\\L%iData\\L%i_%i.PAL", l, l, xrandom(0, 4) + 1);
...
#ifndef FASTER // This keeps the menu from rendering so quickly that it blinks 
			   // The menu will refresh too quickly and will be uncontrolable withtout this.
