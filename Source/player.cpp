...
void InitPlayer(int pnum, BOOL FirstTime)
{
	...
	if (plr[pnum].plrlevel == currlevel || leveldebug) {
		...
		if (plr[pnum]._pHitPoints >> 6 > 0) {
			...
			plr[pnum]._pAnimFrame = xrandom(2, plr[pnum]._pNFrames - 1) + 1;
			plr[pnum]._pAnimCnt = xrandom(2, 3);
		} else {
