	...
	if (leveltype == DTYPE_CATHEDRAL) {
		...
		for (i = 0; i < 256 && numthemes < MAXTHEMES; i++) {
			if (CheckThemeRoom(i)) {
				...
				for (j = ThemeGood[xrandom(0, 4)];; j = xrandom(0, 17)) {
...
void PlaceThemeMonsts(int t, int f)
{
	...
	mtype = scattertypes[xrandom(0, numscattypes)];
	for (yp = 0; yp < MAXDUNY; yp++) {
		for (xp = 0; xp < MAXDUNX; xp++) {
			if (dTransVal[xp][yp] == themes[t].ttval && !nSolidTable[dPiece[xp][yp]] && dItem[xp][yp] == 0 && dObject[xp][yp] == 0) {
				if (xrandom(0, f) == 0) {
					AddMonster(xp, yp, xrandom(0, 8), mtype, 1);
				}
			}
		}
	}
...
void Theme_Barrel(int t)
{
	...
	for (yp = 0; yp < MAXDUNY; yp++) {
		for (xp = 0; xp < MAXDUNX; xp++) {
			if (dTransVal[xp][yp] == themes[t].ttval && !nSolidTable[dPiece[xp][yp]]) {
				if (xrandom(0, barrnd[leveltype - 1]) == 0) {
					r = xrandom(0, barrnd[leveltype - 1]) != 0;
					r += OBJ_BARREL;
					AddObject(r, xp, yp);
				}
			}
...
void Theme_SkelRoom(int t)
{
	...
	if (xrandom(0, monstrnd[leveltype - 1]) != 0) {
		i = PreSpawnSkeleton();
...
void Theme_Treasure(int t)
{
	...
	for (yp = 0; yp < MAXDUNY; yp++) {
		for (xp = 0; xp < MAXDUNX; xp++) {
			if (dTransVal[xp][yp] == themes[t].ttval && !nSolidTable[dPiece[xp][yp]]) {
				int rv = xrandom(0, treasrnd[leveltype - 1]);
				// BUGFIX: the `2*` in `2*xrandom(0, treasrnd...) == 0` has no effect, should probably be `xrandom(0, 2*treasrnd...) == 0`
				if ((2 * xrandom(0, treasrnd[leveltype - 1])) == 0) {
...
void Theme_Library(int t)
{
	...
	for (yp = 1; yp < MAXDUNY - 1; yp++) {
		for (xp = 1; xp < MAXDUNX - 1; xp++) {
			if (CheckThemeObj3(xp, yp, t, -1) && dMonster[xp][yp] == 0 && xrandom(0, librnd[leveltype - 1]) == 0) {
				AddObject(OBJ_BOOKSTAND, xp, yp);
				if (xrandom(0, 2 * librnd[leveltype - 1]) != 0) {
					oi = dObject[xp][yp] - 1;
		...
		for (xp = 1; xp < MAXDUNX - 1; xp++) {
			if (dTransVal[xp][yp] == themes[t].ttval && !nSolidTable[dPiece[xp][yp]]) {
				if (CheckThemeObj3(xp, yp, t, -1)) {
					if (xrandom(0, tortrnd[leveltype - 1]) == 0) {
...
void Theme_Decap(int t)
{
	...
	for (yp = 1; yp < MAXDUNY - 1; yp++) {
		for (xp = 1; xp < MAXDUNX - 1; xp++) {
			if (dTransVal[xp][yp] == themes[t].ttval && !nSolidTable[dPiece[xp][yp]]) {
				if (CheckThemeObj3(xp, yp, t, -1)) {
					if (xrandom(0, decaprnd[leveltype - 1]) == 0) {
...
void Theme_ArmorStand(int t)
{
	...
	for (yp = 0; yp < MAXDUNY; yp++) {
		for (xp = 0; xp < MAXDUNX; xp++) {
			if (dTransVal[xp][yp] == themes[t].ttval && !nSolidTable[dPiece[xp][yp]]) {
				if (CheckThemeObj3(xp, yp, t, -1)) {
					if (xrandom(0, armorrnd[leveltype - 1]) == 0) {
...
void Theme_BrnCross(int t)
{
	...
	for (yp = 0; yp < MAXDUNY; yp++) {
		for (xp = 0; xp < MAXDUNX; xp++) {
			if (dTransVal[xp][yp] == themes[t].ttval && !nSolidTable[dPiece[xp][yp]]) {
				if (CheckThemeObj3(xp, yp, t, -1)) {
					if (xrandom(0, bcrossrnd[leveltype - 1]) == 0) {
...
void Theme_WeaponRack(int t)
{
	...
	for (yp = 0; yp < MAXDUNY; yp++) {
		for (xp = 0; xp < MAXDUNX; xp++) {
			if (dTransVal[xp][yp] == themes[t].ttval && !nSolidTable[dPiece[xp][yp]]) {
				if (CheckThemeObj3(xp, yp, t, -1)) {
					if (xrandom(0, weaponrnd[leveltype - 1]) == 0) {
