    ...
    if (wRemoveMsg == PM_NOREMOVE) {
        ...
        switch (e.type) {
            ...
            case SDL_MOUSEBUTTONUP: {
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "DEBUGMSG", "Button %d", button  );

                if (button != SDL_BUTTON_LEFT && button != SDL_BUTTON_RIGHT) {
                    return FALSE;
                }
                bool left = button == SDL_BUTTON_LEFT;
                __android_log_print(ANDROID_LOG_DEBUG, "DEBUGMSG", "Left %d", left  );

                lpMsg->message = e.type == SDL_MOUSEBUTTONDOWN ? (left ? WM_LBUTTONDOWN : WM_RBUTTONDOWN)  : (left ? WM_LBUTTONUP : WM_RBUTTONUP);
                lpMsg->lParam = (e.button.y << 16) | (e.button.x & 0xFFFF);

                __android_log_print(ANDROID_LOG_DEBUG, "DEBUGMSG", "LPMSG %d", lpMsg->lParam  );
                __android_log_print(ANDROID_LOG_DEBUG, "DEBUGMSG", "LPMSG %d", lpMsg->message );
